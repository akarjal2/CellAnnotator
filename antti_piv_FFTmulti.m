function [xtable, ytable, utable, vtable, typevector] = antti_piv_FFTmulti (image1,image2,interrogationarea, step, subpixfinder, mask_inpt, roi_inpt,passes,int2,int3,int4,imdeform)
%profile on
...
%         fprintf('.');
    ...
    %d.h. u und v auf die gewünschte größe bringen+interpolieren
    if GUI_avail==1
...
%                 fprintf('.');
        ...
        for i = minix:step:maxix % horizontal loop
            ...
            %step ist 8, für 32px intare, sollte es aber 16 sein, sonst gehts nicht
